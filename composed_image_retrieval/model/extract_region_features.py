...
class extract_region_feature(nn.Module):
    ...
    def get_inputs(self,cfg, image,text):
    ...
    def forward(self,img,text):
        batched_inputs=self.get_input(self.cfg,img,text)
        ...
        box_features = self.model.roi_heads._shared_roi_transform(
            [features[f] for f in self.model.roi_heads.in_features], proposal_boxes, model.backbone.layer4
        )
